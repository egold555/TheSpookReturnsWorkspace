...
public abstract class EntityMoveable extends Entity{
	...
	public void damage(int damage) {
		...
		health -= damage;
		//GELog.info("[" + this.getClass().getName() + "] Took " + damage + " damage | Health: " + health);
		if(health <= 0) {
			onDeath();
		}
	}
	...
	public void move(Terrain terrain) {
		...
		float frameTime= GameEngine.getInstance().getDisplayManager().getFrameTimeSeconds();
		...
		if (hasGravity) {
			velocity.y += getGravity() * frameTime;
		}
...
*/
	...
	private List<Entity> getIntersectingEntities(float x, float y, float z)
	{
		...
		for (Collider myCollider: getColliders()) {
			...
			for (Entity entity: GameEngine.getInstance().getEntities())
			{
