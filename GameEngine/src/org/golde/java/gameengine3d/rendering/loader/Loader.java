...
import org.golde.java.gameengine3d.texturing.skybox.SkyboxTextureData;
...
public class Loader {
	...
	public int loadCubeMap(String[] texturesFiles) {
		...
		for(int i = 0; i < texturesFiles.length; i++) {
			SkyboxTextureData data = decodeTextureFile("res/skybox/" + texturesFiles[i] + ".png");
	...
	private SkyboxTextureData decodeTextureFile(String fileName) {
		int width = 0;
		int height = 0;
		...
		try {
			...
			GELog.error(e, "Tried to load texture " + fileName + ", didn't work");
			System.exit(-1);
		}
		return new SkyboxTextureData(buffer, width, height);
	}
	...
	public int loadTexture(String fileName, boolean withExtention) {
		...
		try {
			...
			GELog.error("Texture 'res/" + fileName + ".png' does not exist!");
