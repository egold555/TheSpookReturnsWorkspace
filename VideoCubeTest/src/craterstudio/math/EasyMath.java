...
public class EasyMath {
	...
	public static final float transform(float ratio, int type) {
		...
		if (type == INVERSE_SQUARED_TRANSFORM
				|| type == INVERSE_SQUARED_SMOOTH_TRANSFORM)
			ratio = 1.0f - ratio;
	...
	public static final void map(float[] arr, float min, float max) {
		float gotMin = Integer.MAX_VALUE;
		float gotMax = Integer.MIN_VALUE;

		for (float v : arr) {
			gotMin = Math.min(gotMin, v);
			gotMax = Math.max(gotMax, v);
		}

		for (int i = 0; i < arr.length; i++) {
			float v = arr[i];
			v = (v - gotMin) / (gotMax - gotMax);
			v = min + v * (max - min);
			arr[i] = v;
		}
	}
    ...
    */
	...
	public static final float gradients(float cur, float[] keys, float values[]) {
		...
		for (int i = 1; i < keys.length; i++) {
			if (cur < keys[i]) {
				...
				return EasyMath.interpolate(cur, keys[src], keys[dst],
	...
	public static final double curve(double distance, double topDistance,
			double margin, double power) {
	...
	public static final float interpolate(float cur, float min, float max,
			float startValue, float endValue) {
		return lerp(startValue, endValue, invLerp(cur, min, max));
	}

	public static final float interpolate(float cur, float min, float max,
			float startValue, float endValue, int transform) {
		return lerp(startValue, endValue,
				transform(invLerp(cur, min, max), transform));
	}

	public static final float interpolateWithCap(float cur, float min,
			float max, float startValue, float endValue) {
		if (cur < min)
			return startValue;
		if (cur > max)
			return endValue;
	...
	public static final float interpolate2d(float x, float z, float nw,
			float ne, float se, float sw) {
